"""Модуль мониторинга баланса ресурсов магазина.

С помощью OCR считывает балансы валют в различных магазинах (алмазы, монеты, медали, очки заслуг,
монеты гильдии, данные ядра, ваучеры и др.) и синхронизирует данные с Dashboard.
Подклассы магазинов наследуют ShopStatus для получения текущего количества валюты.
"""

# Этот файл предназначен для отслеживания остатков ресурсов в обычных магазинах, включая магазин медалей, заслуг и ядерных данных.
# Значения алмазов, монет и различных медалей считываются через OCR и синхронизируются с Dashboard посредством LogRes.
...
class ShopStatus(UI):
    """Считыватель баланса валюты магазина.

    Предоставляет интерфейс OCR-распознавания типов валют магазинов, синхронизируя
    баланс с LogRes Dashboard. Подклассы магазинов наследуют этот класс для получения
    реализации shop_currency() по умолчанию.

    Attributes:
        _currency (int): Кэш текущего баланса валюты.
    """
