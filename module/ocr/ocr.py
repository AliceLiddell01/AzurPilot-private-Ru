...
def normalize_ocr_text(model_name: str, text: str) -> str:
    """Убирает ложные пробелы в компактных значениях английской OCR-модели.

    Для остальных моделей строка возвращается без изменений. Обычные пробелы
    в названиях и фразах сохраняются.
    """
