"""Бэкенд OCR-распознавания NCNN.

Модель распознавания текста на базе фреймворка NCNN, обеспечивающая более высокую скорость инференса, чем ONNX.
NCNN — это высокопроизводительный фреймворк нейросетевого инференса, оптимизированный под мобильные платформы,
особенно эффективный в сценариях выполнения на CPU.

Спецификация модели:
- Вход: 3-канальное RGB-изображение 48x320
- Выход: последовательность текста после CTC-декодирования
- Файлы модели: .param (структура сети) + .bin (веса) + файл словаря

Поддерживаемые модели:
- azur_lane: распознавание Global/English игрового текста

Примечание: бэкенд ncnn не поддерживает детекцию текста, используется совместно с моделью детекции ONNX.
"""
...
def _destroy_gpu_instance():
    """Обработчик atexit: безопасное уничтожение глобального экземпляра GPU ncnn."""
