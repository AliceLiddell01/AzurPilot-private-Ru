...
class BotRuntimeWorkerManager:
    _processes: dict[str, "BotRuntimeWorkerManager"] = {}
    ...
    def start(
        ...
        *,
        ...
        with self._get_lifecycle_lock(self.config_name):
            ...
            process = Process(
                target=BotRuntimeWorkerManager.run_process,
                args=(
                    self.config_name,
                    func,
                    ev,
        ...
        if os.name == "nt":
            try:
                ...
                if result.returncode == 0:
                    return BotRuntimeWorkerManager._wait_pid_exit(pid, timeout=3)
                if not BotRuntimeWorkerManager._pid_exists(pid):
                    return True
                logger.warning(f"[Bot Runtime] Не удалось остановить рабочий процесс PID {pid}: taskkill вернул код {result.returncode}")
                return False
            except (OSError, subprocess.TimeoutExpired) as exc:
                logger.warning(f"[Bot Runtime] Не удалось остановить рабочий процесс PID {pid}: {exc}")
                return False
        else:
            try:
                import psutil
    ...
    def _wait_pid_exit(pid: int, timeout: float) -> bool:
        ...
        while time.time() < deadline:
            if not BotRuntimeWorkerManager._pid_exists(pid):
                return True
            time.sleep(0.1)
        return not BotRuntimeWorkerManager._pid_exists(pid)
    ...
    def state(self) -> int:
        ...
        try:
            ...
            return 3
        return 3 if snapshot is not None and snapshot.phase is RuntimePhase.FAILED else 2

    @classmethod
    def get_manager(cls, config_name: str) -> "BotRuntimeWorkerManager":
        """
        ...
        with cls._managers_lock:
            if config_name not in cls._processes:
                cls._processes[config_name] = BotRuntimeWorkerManager(config_name)
    ...
    def run_process(
        ...
        if not BotRuntimeWorkerManager._wait_for_runtime_state_registration(
            config_name,
            repository_root,
        ):
            return
        ...
        heartbeat = threading.Thread(
            target=BotRuntimeWorkerManager._runtime_state_heartbeat,
            args=(
                config_name,
                repository_root,
                operation_id,
        ...
        heartbeat.start()
        try:
            BotRuntimeWorkerManager._run_process_body(config_name, func, e)
        finally:
            heartbeat_stop.set()
    ...
    def _run_process_task_body(
        ...
        if os.environ.get("DEMO") == "1":
            logger.info("[Bot Runtime] Тестовая запись 3")
            time.sleep(1)
            logger.info("[Bot Runtime] Тестовая запись 2")
            time.sleep(1)
            logger.info("[Bot Runtime] Тестовая запись 1")
            time.sleep(1)
            logger.info("[Bot Runtime] Эта версия предназначена для демонстрации")
            return
        ...
        try:
            ...
            if func == "alas":
                ...
                if mod is None:
                    logger.critical(f"[Bot Runtime] Не удалось загрузить функциональный модуль: {func}")
                    return
                ...
                logger.critical(
                    f"[Bot Runtime] Функциональный модуль не найден: {func}"
                )
