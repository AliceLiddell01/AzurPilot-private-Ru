...
записи. Для PT строка Dashboard хранит накопительный счётчик события; текущий
...
Структура данных панели:
    Dashboard.<имя ресурса> = {
        'Value': int,       # Текущее значение либо накопительный PT события.
...
class LogRes:
    ...
    def _record_event_pt(self, value):
        """Сохранить накопительный PT и безопасно разбудить магазин события."""
        try:
