        ...
        try:
            with device_id_file.open('r', encoding='utf-8') as f:
                ...
                if stored_id and stored_id != device_id:
                    ...
                    logger.info(f'Обнаружено изменение ID устройства для миграции! Старый: {stored_id[:8]}, новый: {device_id[:8]}')
        except Exception:
            pass
...
def _overwrite_device_id(device_id: str, file_path: Path):
    try:
        ...
        logger.warning(f'[Устройство — ID] Не удалось перезаписать файл ID устройства: {e}')
