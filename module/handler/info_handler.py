...
Панель уведомлений (Info Bar):
    Полоса уведомлений в верхней части экрана, содержащая сообщения о завершении заказов,
...
class InfoHandler(ModuleBase):
    ...
    - Обработка срочных заказов (handle_urgent_commission)
    ...
    def handle_urgent_commission(self, drop=None):
        """
        Обработать всплывающее окно срочного заказа.

        Args:
            drop: Объект записи изображений дропа, может быть None.

        Returns:
            Обнаружено и обработано ли всплывающее окно срочного заказа.
        """
        appear = self.appear(GET_MISSION, offset=True, interval=2)
        if appear:
            logger.info('[Обработчик — заказы] Получен срочный заказ')
            if drop:
