...
# Этот файл предоставляет низкоуровневые вспомогательные функции для WebUI.
# Содержит чтение/запись LocalStorage, инъекцию JavaScript-кода, управление стилями CSS, форматирование времени и контроллер планирования задач для обновления UI.
import base64
import datetime
import io
import operator
...
DARK_TERMINAL_THEME = TerminalTheme(
    (30, 30, 30),  # Цвет фона
    (204, 204, 204),  # Цвет переднего плана
    [
        (0, 0, 0),  # Черный
        (205, 49, 49),  # Красный
        (13, 188, 121),  # Зеленый
    ...
    [  # Яркий
        ...
        (245, 245, 67),  # Желтый
        (59, 142, 234),  # Синий
        (214, 112, 214),  # Пурпурный / маджента
        (41, 184, 219),  # Голубой
        (229, 229, 229),  # Белый
    ],
)
...
def parse_pin_value(val, valuetype: str = None, widget_type: str = None, options=None):
    ...
    # Обработка типа dict - извлечение поля 'value' и рекурсивный разбор
    if isinstance(val, dict):
        if 'value' in val:
            return parse_pin_value(val['value'], valuetype, widget_type, options)
        else:
            # Возврат исходного dict при отсутствии ключа 'value'
            return val
        ...
        if len(val) == 0:
            return []
        # Различие между checkbox ([True]) и multiselect ([3, 1, 5])
        # Значение checkbox всегда [True] или [], непустой список элементов кроме bool считается multiselect
...
# Monkey-patch: замена стандартного обработчика исключений PyWebIO
