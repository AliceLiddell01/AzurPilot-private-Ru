"""
Модуль операций в интерфейсе дока (Dock).

Предоставляет функции взаимодействия с интерфейсом дока, включая карточную сетку кораблей, переключение сортировки,
фильтрацию избранных, настройку и подтверждение фильтров, выбор кораблей, переход в окно сведений о корабле и т. д.
Определяет сетки кнопок для зон атрибутов карточек (редкость, уровень, настроение, индикатор настроения)
и цветовые константы настроения.
"""
...
class Dock(Equipment):
    """Обработчик операций в интерфейсе дока.

    Предоставляет общие операции на странице дока: ожидание загрузки карточек, переключение сортировки, фильтрацию избранных,
    настройку фильтров, выбор и подтверждение кораблей, переход в подробные сведения о корабле и др.
    Наследует Equipment для повторного использования боковой панели снаряжения.

    Различия серверов: на TW-сервере расположение кнопок фильтра отличается от других серверов,
    что диспетчеризируется декоратором @Config.when.
    """
    def handle_dock_cards_loading(self, skip_first_screenshot=True):
        """
        Ожидает завершения загрузки карточек в доке.

        Определяет стабильность изображения путём сравнения хешей двух последовательных кадров;
        если док пуст, немедленно выходит.
        Использует Timer(1.2s) в качестве запасного тайм-аута, confirm_timer использовать невозможно.

        Args:
            skip_first_screenshot: Пропускать ли первый снимок экрана, повторно используя снимок из предыдущего цикла состояния.
        """
