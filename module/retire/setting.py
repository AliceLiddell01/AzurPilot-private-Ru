"""Модуль управления настройками отставки (списания) кораблей.

Управляет операциями в интерфейсе настроек быстрого списания, включая вход и выход со страницы настроек,
переключение опций списания (диапазон редкости для списания, правила сохранения и т. д.).

QuickRetireSetting расширяет класс Setting, адаптируя его под визуальный стиль настроек списания.
QuickRetireSettingHandler предоставляет навигационные операции для страницы настроек.

Наследует UI для использования возможностей перехода по страницам.
"""
...
class QuickRetireSetting(Setting):
    """Элемент настроек быстрого списания.

    Определяет статус активации опций списания.
    """
...
class QuickRetireSettingHandler(UI):
    """Обработчик навигации по странице настроек списания.

    Предоставляет операции входа и выхода со страницы настроек списания.
    """
