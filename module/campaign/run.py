...
- обработку уведомлений о заказах.
...
class CampaignRun(CampaignEvent, ShopStatus):
    ...
    def handle_commission_notice(self):
        """Обработать уведомление о завершившемся заказе.

        Если уведомление найдено, текущая задача останавливается и вызывается
        обработчик заказов.

        Исключения:
            TaskEnd: выбрасывается после обнаружения уведомления о заказе.

        Страницы:
            вход: page_campaign
        """
        if self.config.is_task_enabled('Commission') and self.campaign.commission_notice_show_at_campaign():
            logger.info('[Кампания — запуск] Обнаружено уведомление о заказе')
            self.config.task_call('Commission')
            self.config.task_stop('Обнаружено уведомление о заказе')
