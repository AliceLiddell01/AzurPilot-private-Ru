"""
Модуль анализа и выбора соперников в учениях.

С помощью OCR распознаёт уровень и силу флота соперников, а затем сортирует их в соответствии с выбранной стратегией.

Поддерживаемые стратегии выбора соперника:
- max_exp: сортировка по сумме уровней с приоритетом атаки соперника с наибольшим опытом
- easiest: сортировка по комплексной сложности с приоритетом наименее сильного соперника
    (чем ниже уровень и сила флота соперника, тем выше приоритет)
- leftmost: по порядку по умолчанию (наивысший приоритет у крайнего левого)

Константы:
    MAX_LVL_SUM: максимальная сумма уровней (6 кораблей * 125 уровень = 750)
    PWR_FACTOR: коэффициент нормализации силы флота для масштабирования к сопоставимому диапазону
"""
import numpy as np
...
class Level(Digit):
    """
    OCR-распознаватель уровня соперника в учениях.

    Выполняет предобработку цифрового OCR: обрезает пустое пространство слева и добавляет белые поля
    для повышения точности распознавания чисел уровня на экране подготовки к учениям.
    """
...
class Opponent:
    """
    Класс данных соперника в учениях, содержащий сведения об уровнях и силе его флота.

    Распознаёт через OCR по снимкам экранов учений уровень и силу двух флотов соперника
    и рассчитывает приоритетный балл соперника по заданной стратегии.

    Attributes:
        index (int): Индекс соперника (0–3, слева направо).
        power (list[int]): Сила двух флотов, например [14848, 13477].
        level (list[int]): Уровни шести кораблей, например [120, 120, 120, 120, 120, 120].
    """
...
class OpponentChoose(UI):
    """
    Селектор соперников, отвечающий за проверку и сортировку соперников в учениях.

    Последовательно заходит на экран подготовки каждого соперника, считывает через OCR уровень флота и силу,
    затем сортирует соперников по выбранной стратегии и возвращает список приоритетов атаки.

    Attributes:
        main_image (numpy.ndarray): Снимок главной страницы учений для распознавания силы.
        opponents (list[Opponent]): Список объектов Opponent для четырёх соперников.
    """
    ...
    def _opponent_fleet_check_all(self):
        """
        Последовательная проверка информации о флотах всех 4 соперников.

        Переходит на экран подготовки каждого соперника кликом, распознаёт уровень и силу через OCR и возвращается.
        """
        self.opponents = []
