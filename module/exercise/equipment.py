"""Модуль управления снаряжением в учениях.

Управляет операциями редактирования снаряжения в учениях (PvP).
На экране подготовки к учениям позволяет переключать конфигурации снаряжения кораблей.

Функции:
- Включение/отключение режима редактирования снаряжения
- Проверка состояния кнопки редактирования снаряжения

Наследуется от базового класса Equipment.
"""
...
class ExerciseEquipment(Equipment):
    """Редактор снаряжения в учениях.

    Управляет переключением состояния кнопки редактирования снаряжения на экране подготовки к учениям.
    """
