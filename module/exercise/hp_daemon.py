"""Модуль мониторинга здоровья в учениях в реальном времени.

В боях учений (PvP) в реальном времени отслеживает процент HP атакующей и защищающейся сторон.
С помощью анализа распределения цветов полосы HP рассчитывает текущий уровень здоровья.

Анализ цветов полосы HP:
- Зелёная область: достаточное количество здоровья
- Красная область: низкий уровень здоровья
- Процент HP рассчитывается по доле цвета (от 0.0 до 1.0)

Сценарии использования:
- Запуск защитной логики (отступления) при критически низком уровне здоровья союзников
- Фиксация динамики изменения здоровья в ходе боя

Наследуется от ModuleBase, используется в Exercise.
"""
...
class HpDaemon(ModuleBase):
    """Демон мониторинга здоровья в учениях.

    В реальном времени отслеживает состояние HP атакующей и защищающейся сторон в бою учений.

    Attributes:
        attacker_hp (float): Процент HP атакующей (союзной) стороны (0.0-1.0).
        defender_hp (float): Процент HP защищающейся (вражеской) стороны (0.0-1.0).
        low_hp_confirm_timer (Timer): Таймер подтверждения низкого HP для защиты от ложных срабатываний.
    """
    attacker_hp = 1.0
    defender_hp = 1.0
    # _last_secure_time = 0
