...
def tool_catalog_sha256(tool_names: Iterable[str] | None = None) -> str:
    """Хешировать канонический каталог имён и публичных схем инструментов.

    Каноническая форма не зависит от порядка публикации и включает полные
    публичные описания `Tool`; вызов совместимости с неполным набором имён
    сохраняет проверяемую совместимость только по именам.
    """
