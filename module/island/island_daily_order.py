...
class IslandDailyOrder(Island):
    ...
    def run(self):
        logger.hr('Ежедневные заказы острова', level=1)
        ...
        if urgent_remaining is None:
            logger.warning('[Остров — ежедневные заказы] Не удалось распознать оставшееся число срочных поручений на этой неделе; проверка срочных поручений сохранена')
    ...
    def _ocr_urgent_remaining(self):
        ...
        try:
            ...
            logger.warning('[Остров — ежедневные заказы] Ошибка OCR оставшегося числа срочных поручений на этой неделе')
            return None

        if total != self.URGENT_TOTAL_COUNT:
            logger.warning(f'[Остров — ежедневные заказы] OCR вернул некорректное общее число срочных поручений: {current}/{total}')
            return None

        logger.info(f'[Остров — ежедневные заказы] Оставшиеся срочные поручения на этой неделе: {current}/{total}')
        return current
        ...
        if isinstance(value, datetime):
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f'[Остров — ежедневные заказы] Некорректный формат времени обновления срочных поручений: {value}')
                    return None
        else:
            logger.warning(f'[Остров — ежедневные заказы] Некорректный тип времени обновления срочных поручений: {type(value).__name__}')
            return None
    ...
    def _submit_order(self, button, must_appear=False):
        ...
        """
        if button is None:
            logger.warning('[Остров — ежедневные заказы] Кнопка доставки не настроена')
            return None
        if must_appear and not self.appear(button):
            logger.warning(f'[Остров — ежедневные заказы] Кнопка доставки не обнаружена: {button}')
            return None
        self.device.click(button)
        self.device.sleep(self.FAST_POPUP_CHECK_INTERVAL)
        self.device.screenshot()
        if self.appear(POPUP_RESOURCE_INSUFFICIENT, offset=30):
            logger.info('[Остров — ежедневные заказы] Недостаточно ресурсов для заказа')
            self.device.sleep(3)
            return False
    ...
    def _step_urgent(self):
        ...
        if urgent_match:
            ...
            logger.info('[Остров — ежедневные заказы] Срочное поручение обнаружено по фиксированной позиции')
        else:
            logger.info('[Остров — ежедневные заказы] Значок срочного поручения не обнаружен; переход к шагу ②')
            return 'next'
    ...
    def _step_right_panel(self):
        ...
        if self._is_right_panel_empty():
            if self._first_right_panel_check:
                ...
                logger.info('[Остров — ежедневные заказы] При первом входе правая панель пуста; задержка до следующих 03:00')
                return 'next_day'
            else:
                logger.info('[Остров — ежедневные заказы] Правая панель пуста; выход и повторный вход')
                return 'reenter'
        ...
        if self._check_items_for_reject():
            ...
            logger.info('[Остров — ежедневные заказы] Попытка доставки заказа')
            if self._submit_order(DAILY_ORDER_DELIVER):
                ...
                if self._is_right_panel_empty():
                    ...
                    logger.info('[Остров — ежедневные заказы] После доставки правая панель не пуста; выход и повторный вход для обновления состояния')
                    return 'reenter'

            logger.info('[Остров — ежедневные заказы] Недостаточно ресурсов для заказа; отклонение')
    ...
    def _step_challenge_easy(self):
        ...
        for template, label in [
            ...
            (TEMPLATE_DAILY_ORDER_EASY, 'лёгкий'),
        ]:
        ...
        if not all_matches:
            if self.appear_then_click(DAILY_ORDER_CHALLENGE_EASY_SPECIAL_CHECK, interval=2):
                logger.info('[Остров — ежедневные заказы] Поручение «сложное/лёгкое» обнаружено по фиксированной позиции')
                self.device.sleep(1)
                self.device.screenshot()
                if self._is_preparing():
                    logger.info('[Остров — ежедневные заказы] Поручение «сложное/лёгкое» готовится; переход к проверке выхода')
                    return 'to_step4'
                elif self._has_reject_button():
                    logger.info('[Остров — ежедневные заказы] Поручение «сложное/лёгкое» выбрано; переход к шагу ② для попытки доставки')
                    return 'to_step2'
                else:
                    logger.warning('[Остров — ежедневные заказы] Состояние поручения «сложное/лёгкое» неизвестно; переход к проверке выхода')
                    return 'to_step4'
        ...
        for pos, template, label in all_matches:
            ...
            logger.info(f'[Остров — ежедневные заказы] Обработка значка «{label}» (pos={pos_key})')
            ...
            if self._is_preparing():
                ...
                logger.info(f'[Остров — ежедневные заказы] Поручение «{label}» выбрано; переход к шагу ② для попытки доставки')
                return 'to_step2'
            else:
    ...
    def _step_exit(self):
        ...
        if self._is_preparing():
            logger.info('[Остров — ежедневные заказы] На правой панели есть готовящийся заказ; OCR времени ожидания')
            seconds = self._ocr_cooldown_seconds()
            if seconds and seconds > 0:
                target = current_time() + timedelta(seconds=seconds)
                self.config.task_delay(target=target)
                logger.info(f'[Остров — ежедневные заказы] Ожидание подготовки: {seconds} сек.')
            else:
                logger.warning('[Остров — ежедневные заказы] Не удалось распознать время подготовки; используется 1 час')
    ...
    def _template_click_urgent(self, similarity=0.75):
        ...
        for name, template in self._urgent_templates():
            ...
            if not match:
                continue
            logger.info(f'[Остров — ежедневные заказы] Совпадение шаблона срочного поручения: {name}')
            mx, my, tw, th = match
    ...
    def _enter_daily_order(self):
        """从岛屿手机页面进入每日订单界面。"""
        logger.info('[Остров — ежедневные заказы] Вход в интерфейс ежедневных заказов')
        while 1:
            self.device.screenshot()
            if self.appear(DAILY_ORDER_CHECK):
                logger.info('[Остров — ежедневные заказы] Интерфейс ежедневных заказов открыт')
                break
    ...
    def _back_to_island_phone(self):
        """点击返回按钮回到岛屿手机页面。"""
        logger.info('[Остров — ежедневные заказы] Возврат на страницу телефона острова')
        while 1:
            self.device.screenshot()
            if self.ui_page_appear(page_island_phone):
                logger.info('[Остров — ежедневные заказы] Возврат на страницу телефона острова завершён')
                break
    ...
    def _delay_to_next_daily_run(self):
        ...
        self.config.task_delay(target=target)
        logger.info(f'[Остров — ежедневные заказы] Следующий запуск ежедневных заказов: {target}')

    def _handle_popups(self):
        """处理弹窗。"""
        if self.appear(POPUP_RESOURCE_INSUFFICIENT, offset=30):
            logger.info('[Остров — ежедневные заказы] Окно нехватки ресурсов; ожидание автоматического закрытия')
            self.device.sleep(3)
            return True
        if self.appear(POPUP_ORDER_CANNOT_REPLACE, offset=30):
            logger.info('[Остров — ежедневные заказы] Окно «сейчас заменить нельзя»; ожидание автоматического закрытия')
            self.device.sleep(3)
            return True
        if self.appear(DAILY_ORDER_LEVEL_UP):
            logger.info('[Остров — ежедневные заказы] Обнаружено повышение уровня заказов; закрытие через безопасную область')
    ...
    def _check_items_for_reject(self):
        ...
        for slot_index, slot_area in enumerate(self.ITEM_SLOT_AREAS):
            ...
            if reject_cheese and \
                ...
                logger.info(f'[Остров — ежедневные заказы] В слоте {slot_index + 1} обнаружен сыр')
                return True
            if reject_tofu and \
                    TEMPLATE_TOFU.match(slot_image, similarity=0.80):
                logger.info(f'[Остров — ежедневные заказы] В слоте {slot_index + 1} обнаружен тофу')
                return True
        return False
