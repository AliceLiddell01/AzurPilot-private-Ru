...
class IslandRancher(Island, WarehouseOCR, LoginHandler):
    ...
    def process_mill_item(self, mill_item, quantity=None):
        ...
        logger.info(f"[Остров — ранчо] Переработка {mill_item} x{target}")
        for _ in self.loop(timeout=10, skip_first=False):
            ...
            logger.warning(f"[Остров — ранчо] Истекло время открытия окна переработки мельницы: {mill_item}")
            return False
        ...
        for _ in self.loop(timeout=15, skip_first=False):
            ...
            logger.warning(f"[Остров — ранчо] Истекло время подтверждения переработки мельницы: {mill_item}")
            return False
    ...
    def check_feed_needs(self, target_quantity=50):
        ...
        for post_id, feed_item in self.ranch_feed_map.items():
            ...
            if current_quantity < target_quantity:
                logger.info(f"[Остров — ранчо] Недостаточный запас {feed_item}: {current_quantity}/{target_quantity}")
    ...
    def check_mill_supplement_needs(self, feed_target_quantity=50):
        ...
        if wheat_flour_count < 150:
            ...
            logger.info(f"[Остров — ранчо] Недостаточный запас wheat_flour: {wheat_flour_count}/150; требуется переработать {target}, чтобы довести до 200")
        ...
        for _, feed_item in self.ranch_feed_map.items():
            ...
            if current_quantity < feed_target_quantity:
                ...
                logger.info(f"[Остров — ранчо] Недостаточный запас {feed_item}: {current_quantity}/{feed_target_quantity}; фиксированная переработка {target} партий")
    ...
    def process_mill_supplements(self, feed_target_quantity=50):
        ...
        if not mill_needs:
            logger.info("[Остров — ранчо] Запас корма и муки достаточен")
            return []

        logger.info(f"[Остров — ранчо] Требуется пополнить на мельнице: {mill_needs}")
        ...
        if not self.goto_mill_from_any_ranch_post():
            logger.warning("[Остров — ранчо] Ни через одну из четырёх позиций ранчо нельзя войти на мельницу; пополнение корма и муки пропущено")
            return []
        ...
        for mill_item, quantity in mill_needs:
            if self.process_mill_item_with_inventory(mill_item, quantity):
                ...
                continue
            logger.warning(f"[Остров — ранчо] Не удалось пополнить {mill_item}; пропуск")

        if not self.back_to_postmanage_after_mill_purchase():
            logger.warning("[Остров — ранчо] Не удалось вернуться к управлению позициями после пополнения мельницы")
    ...
    def process_mill_item_with_inventory(self, mill_item, quantity):
        ...
        if current_material is not None and current_material < material_needed:
            logger.info(
                f"Недостаточно {required_material}; невозможно переработать {mill_item}: {current_material}/{material_needed}"
            )
            return False
        ...
        if required_material in self.inventory_counts['farm']:
            ...
            logger.info(f"[Остров — ранчо] Списано сырьё {required_material}: {material_needed} ед.")
        ...
        logger.info(f"[Остров — ранчо] Переработка завершена: {mill_item} +{self.mill_output_quantity(mill_item, quantity)}")
        return True
    ...
    def back_to_postmanage_after_mill_purchase(self):
        ...
        logger.warning("[Остров — ранчо] Истекло время возврата к управлению позициями после работы с мельницей")
        return False
    ...
    def goto_mill_from_any_ranch_post(self):
        for post_id in self.posts_ranch:
            logger.info(f"[Остров — ранчо] Попытка войти на мельницу через позицию ранчо {post_id}")
            if self.goto_mill_from_ranch_post(post_id):
                logger.info(f"[Остров — ранчо] Вход на мельницу через позицию ранчо {post_id} выполнен")
                return True
    ...
    def goto_mill_from_ranch_post(self, post_id):
        ...
        if post_button is None:
            logger.warning(f"[Остров — ранчо] Неизвестная позиция ранчо: {post_id}")
            return False

        self.post_close()
        if not self.post_open(post_button):
            logger.info(f"[Остров — ранчо] Позиция ранчо {post_id} не открыта; войти на мельницу через сведения назначения невозможно")
            return False
        add_opened = False
        for _ in self.loop(timeout=30, skip_first=False):
            ...
            if self.appear(ISLAND_WORKING):
                logger.info(f"[Остров — ранчо] Позиция ранчо {post_id} занята; войти на мельницу через сведения назначения невозможно")
                self.device.click(POST_CLOSE)
                return False
            ...
            if self.appear_then_click(ISLAND_POST_SELECT, offset=1):
                add_opened = True
                logger.info(f"[Остров — ранчо] Позиция ранчо {post_id}: переход к выбору назначения")
                self.device.sleep(0.5)
                continue
            if (
                ...
                logger.info(f"[Остров — ранчо] В позиции ранчо {post_id} нет места для добавления; войти на мельницу через сведения назначения невозможно")
                self.device.click(POST_CLOSE)
                return False
            if self.appear(ISLAND_SELECT_CHARACTER_CHECK, offset=1):
        ...
        logger.warning(f"[Остров — ранчо] Истекло время входа на мельницу через позицию ранчо {post_id}")
        return False
        ...
        for _ in self.loop(timeout=40, skip_first=False):
            ...
            if can_select_post and self.appear_then_click(ISLAND_POST_SELECT, offset=1):
                if in_vacant_post and not add_opened:
                    logger.info(f"[Остров — ранчо] Позиция ранчо {post_id} свободна; переход непосредственно к назначению")
                elif not add_opened:
                    logger.info(f"[Остров — ранчо] Позиция ранчо {post_id}: переход к назначению через повторное назначение")
                add_opened = True
                self.device.sleep(0.5)
                continue
            if self.appear(ISLAND_SELECT_CHARACTER_CHECK, offset=1):
                if self.select_character(character):
                    ...
                    logger.warning(f"[Остров — ранчо] Для назначения на позицию ранчо {post_id} нет доступного персонажа: {character}")
                    self.back_to_postmanage_from_dispatch()
                    return False
                continue
            if self.appear(ISLAND_SELECT_PRODUCT_CHECK, offset=1):
                ...
                if feed_item and self.inventory_counts['mill'].get(feed_item, 0) < 50:
                    logger.info(f"[Остров — ранчо] Запас {feed_item} на складе всё ещё ниже 50; позиция ранчо {post_id} пропущена")
            ...
            if (
                ...
                logger.info(f"[Остров — ранчо] В позиции ранчо {post_id} нет места для добавления; позиция считается заполненной")
            ...
            if (
                ...
                logger.info(f"[Остров — ранчо] В позиции ранчо {post_id} нет места для добавления; существующий слот позиции пропущен")
                self.ranch_last_finish_time = self.ranch_ocr_finish_time(post_id)
                self.device.click(POST_CLOSE)
                return True
            if (
                ...
                return True
        logger.warning(f"[Остров — ранчо] Истекло время получения и добавления назначения для позиции ранчо {post_id}")
        return False
    ...
    def ranch_ocr_finish_time(self, post_id):
        ...
        if (
            ...
            logger.info(f"[Остров — ранчо] Позиция ранчо {post_id} свободна или доступна для сбора; время завершения не записывается")
            return None

        if not self.appear(ISLAND_WORKING):
            logger.info(f"[Остров — ранчо] Работа позиции ранчо {post_id} не подтверждена; время завершения не записывается")
            return None
        ...
        for retry in range(2):
            ...
            if (
                ...
                logger.info(f"[Остров — ранчо] При повторной проверке позиция ранчо {post_id} свободна или доступна для сбора; время завершения не записывается")
                return None
            if not self.appear(ISLAND_WORKING):
                logger.info(f"[Остров — ранчо] После повторной проверки позиция ранчо {post_id} не находится в рабочем состоянии; время завершения не записывается")
                return None
            time_value = time_work.ocr(self.device.image)
            if time_value.total_seconds() > 0:
                finish_time = current_time() + time_value
                logger.info(f"[Остров — ранчо] Позиция ранчо {post_id}: минимальное оставшееся время текущей очереди: {time_value}")
                return finish_time

        logger.warning(f"[Остров — ранчо] Позиция ранчо {post_id}, вероятно, работает, но допустимое оставшееся время не распознано")
        return None
    ...
    def ranch_post(self, post_id, time_var_name):
        ...
        if post_id not in self.posts_ranch:
            logger.error(f"[Остров — ранчо] Неизвестный ID позиции: {post_id}")
            return
        ...
        if self.appear(ISLAND_WORKING) and self.post_mode_check(post_id):
            ...
            logger.warning(f"[Остров — ранчо] Рабочее состояние позиции ранчо {post_id}, возможно, распознано ошибочно; продолжается попытка сбора или добавления назначения")
        ...
        if finish_time is None:
            self.post_close()
            logger.warning(f"[Остров — ранчо] Для позиции ранчо {post_id} не получено допустимое время завершения; таймер позиции не записывается")
            return False
        setattr(self, time_var_name, finish_time)
        self.post_close()
        return True
    ...
    def check_ranch_needs(self):
        ...
        if chicken_count < self.ranch_chicken_threshold:
            ...
            logger.info("[Остров — ранчо] Требуется выполнить задачу выращивания кур")
        ...
        if pork_count < self.ranch_pork_threshold:
            ...
            logger.info("[Остров — ранчо] Требуется выполнить задачу выращивания свиней")

        ranch_needs.append('ISLAND_RANCH_POST3')
        logger.info("[Остров — ранчо] Требуется выполнить задачу выращивания крупного рогатого скота")

        ranch_needs.append('ISLAND_RANCH_POST4')
        logger.info("[Остров — ранчо] Требуется выполнить задачу выращивания овец")
        return ranch_needs
    ...
    def run(self):
        ...
        logger.info("[Остров — ранчо] \nТекущая статистика запасов:")
        logger.info(f"[Остров — ранчо] Запасы фермы: {self.inventory_counts['farm']}")
        logger.info(f"[Остров — ранчо] Запасы мельницы: {self.inventory_counts['mill']}")
        logger.info(f"[Остров — ранчо] Запасы ранчо: {self.inventory_counts['ranch']}")

        logger.info("[Остров — ранчо] \n[3/5] Проверка и пополнение корма и муки...")
        ...
        if processed_mill_items:
            logger.info(f"[Остров — ранчо] В этом запуске пополнено проектов мельницы: {len(processed_mill_items)} — {processed_mill_items}")
        else:
            logger.info("[Остров — ранчо] В этом запуске проекты мельницы не пополнялись")
        ...
        if all_needs:
            ...
            for post_id in all_needs:
                ...
                if time_var_name:
                    ...
                    self.raise_if_island_error()
                    if not success:
                        logger.warning(f"[Остров — ранчо] Не удалось выполнить позицию ранчо {post_id}; пропуск")
                else:
                    logger.warning(f"[Остров — ранчо] Для позиции {post_id} не найдена соответствующая переменная времени")
        else:
            logger.info("[Остров — ранчо] Нет позиций ранчо, требующих выполнения")
        ...
        try:
            ...
            logger.error(f"[Остров — ранчо] Не удалось выполнить задачу рыбного хозяйства: {e}")
            raise
