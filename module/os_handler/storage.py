...
class StorageHandler(GlobeOperation, ZoneManager):
    ...
    def storage_enter(self):
        ...
        Pages:
            ...
            out: STORAGE_CHECK
        """
        logger.info('[Операция «Сирена» — хранилище] Вход в хранилище')
        wait_seconds = 0
        for _ in self.loop():
            # End
            ...
            if self.appear(MISSION_CHECK, offset=(20, 20)):
                logger.warning('[Операция «Сирена» — хранилище] Случайно открыт экран разведданных, попытка выхода')
                self.ui_click(
                    MISSION_QUIT,
    ...
    def storage_quit(self):
        ...
        Pages:
            ...
            out: is_in_map
        """
        logger.info('[Операция «Сирена» — хранилище] Выход из хранилища')
    ...
    def _storage_item_use(self, button):
        ...
        for _ in self.loop():
            ...
            if self.appear(GET_MISSION, offset=True, interval=2):
                logger.info(f'[Операция «Сирена» — хранилище] Использование сведений о предмете -> {GET_MISSION}')
                self.device.click(GET_MISSION)
                self.interval_reset(STORAGE_CHECK)
                get_mission_counter += 1
                if get_mission_counter >= 3:
                    logger.warning('[Операция «Сирена» — хранилище] Возможно, обработка застряла на накопителе энергии; повторное распознавание регистраторов')
                    break
                continue
            # Item rewards
    ...
    def storage_logger_use_all(self):
        ...
        """
        logger.hr('Использование всех регистраторов координат')
        for _ in self.loop():
            ...
            if len(items):
                ...
                logger.info('[Операция «Сирена» — хранилище] Все регистраторы координат в хранилище использованы')
                break

    def logger_use(self):
        logger.hr('Использование регистратора координат')
        self.storage_enter()
        ...
        for sample_type in sample_types:
            for _ in self.loop():
                ...
                if len(items):
                    ...
                    break
        logger.info('[Операция «Сирена» — хранилище] Все образцы в хранилище использованы')

    def tuning_sample_use(self, quit=True):
        logger.hr('Использование преобразованного образца')
        self.storage_enter()
    ...
    def repair_ship_select(self, button, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if self.image_color_count(image, color=(93, 148, 203), count=300):
                logger.info('[Операция «Сирена» — хранилище] Корабль для использования предмета выбран')
                self.interval_clear(STORAGE_FLEET_CHOOSE)
                return True
            if timeout.reached():
                logger.warning('[Операция «Сирена» — хранилище] Истекло время ожидания выбора корабля')
                self.interval_clear(STORAGE_FLEET_CHOOSE)
                return False
    ...
    def repair_pack_use_confirm(self, button, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if self.appear(STORAGE_REPAIR_CONFIRM, offset=(20, 20)) and \
                ...
                logger.info('[Операция «Сирена» — хранилище] Корабль отремонтирован')
    ...
    def storage_checkout_item(self, item, skip_obscure_hazard_2=False, skip_first_screenshot=True):
        ...
        """
        logger.hr('Извлечение предметов из хранилища')
        if SCROLL_STORAGE.appear(main=self):
            if item == 'REPAIR_PACK':
        ...
        for _ in self.loop():
            ...
            if confirm_timer.reached():
                logger.info(f'[Операция «Сирена» — хранилище] В хранилище больше нет предметов {item}.')
                self.storage_quit()
                return False
    ...
    def storage_get_next_item(self, item, use_logger=True, skip_obscure_hazard_2=False):
        ...
        Pages:
            ...
            out: is_in_map, in an obscure/abyssal zone if checkout.
                 ...
                 STORAGE_FLEET_CHOOSE, for using repair packs.
        """
        logger.hr('[Операция «Сирена» — хранилище] Получение следующей скрытой зоны')
        self.storage_enter()
        if use_logger:
            self.storage_logger_use_all()
