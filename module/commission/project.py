"""Модуль разбора информации о заказах.

Отвечает за извлечение атрибутов отдельного заказа из снимка интерфейса заказов,
включая распознавание названия через OCR, определение типа заказа и длительности выполнения,
статуса и извлечение изображения суффикса.

Основной класс Commission инкапсулирует информацию о заказе и использует декоратор
...
и сортировки списка заказов на основе пользовательских правил (например, 'daily_resource-01:30').

Зависимости:
    - module.base.filter: механизм фильтрации по регулярным выражениям
    - module.ocr.ocr: OCR-распознавание текста (Duration, Ocr)
    - module.commission.project_data: словари названий заказов для каждого сервера
"""
...
def crop_suffix_image(image, area):
    """Обрезает изображение суффикса с римской цифрой справа от названия заказа.

    Args:
        image: Снимок экрана игры.
        area: Область названия заказа.
...
class Commission:
    """Информация об отдельном заказе.

    Инкапсулирует атрибуты, извлечённые из снимка экрана интерфейса заказов: название, тип, статус, длительность и др.
    Поддерживает серверы CN, EN, JP, TW, распределяя специфичную логику парсинга через декоратор `@Config.when`.
    """

    # Кнопка входа в детали заказа
    button: Button
    # Название заказа, распознанное OCR
    name: str
    # Успешно ли разобрано название заказа
    valid: bool
    ...
    suffix_hash: str
    # Название типа заказа, определённое в project_data.py
    ...
    duration: timedelta
    # Время до истечения; задаётся только для срочных заказов, иначе None
    expire: timedelta
    ...
    def __init__(self, image, y, config):
        """Разбирает информацию о заказе по снимку экрана.

        Определяет область обрезки карточки заказа по координате y и вызывает commission_parse
        для распознавания атрибутов и рассчитывает поля категории и длительности для фильтра.

        Args:
            image: Снимок экрана игры.
            y: Y-координата нижней границы карточки заказа.
    ...
    def commission_parse(self):
        """Разбирает информацию о заказе (сервер EN).

        На сервере EN названия заказов длиннее, а область OCR отличается от CN.
        ...
        # Время до истечения — только у срочных заказов
    ...
    def commission_parse(self):
        """Разбирает информацию о заказе (сервер JP).
    ...
    def commission_parse(self):
        """Разбирает информацию о заказе (сервер TW).
    ...
    def commission_parse(self):
        """Разбирает информацию о заказе (сервер CN, стандартный резервный вариант).
    ...
    def __str__(self):
        """Возвращает читаемое строковое представление заказа: название, тип, статус и длительность."""
    ...
    def __eq__(self, other):
        """Определяет, описывают ли два объекта один и тот же заказ.

        Выполняет комплексное сравнение по типу, статусу, суффиксу, длительности (допускается погрешность 120 секунд),
        времени до истечения и счётчику повторений. Для срочных заказов с ящиками также сопоставляются теги фракций (NYB/BIW).

        Args:
            other: Сравниваемый объект заказа.

        Returns:
            bool: Описывают ли объекты один и тот же заказ.
        """
    ...
    def __hash__(self):
        """Возвращает хеш-значение заказа на основе его типа и названия."""
        return hash(f'{self.genre}_{self.name}')

    def suffix_match(self, other, similarity=0.75):
        """Определяет, совпадают ли изображения суффиксов двух заказов.
    ...
    def commission_name_parse(self, string):
        """Сопоставляет название заказа с его типом (сервер EN).

        Сначала проверяет, относится ли заказ к событию, затем перебирает словарь EN-названий по ключевым словам.

        Args:
            string: Название заказа, например 'DAILY RESOURCE EXTRACTION'.

        Returns:
            Строка с типом заказа (например, 'urgent_gem') либо пустая строка, если тип не распознан.
        """
    ...
    @Config.when(SERVER='jp')
    def commission_name_parse(self, string):
        """Сопоставляет название заказа с его типом (сервер JP).

        Использует расстояние Левенштейна для нечёткого сопоставления с допуском до 2 ошибочных символов OCR.
        Сначала проверяет, относится ли заказ к событию, затем перебирает словарь JP-названий.

        Args:
            string: Название заказа, например 'Short-distance Practice'.

        Returns:
            Строка с типом заказа (например, 'extra_drill') либо пустая строка, если тип не распознан.
        """
        if self.is_event_commission():
            return 'daily_event'
        import jellyfish
    ...
    @Config.when(SERVER='tw')
    def commission_name_parse(self, string):
        """Сопоставляет название заказа с его типом (сервер TW).

        Сначала проверяет, относится ли заказ к событию, затем перебирает словарь TW-названий по ключевым словам.

        Args:
            string: Название заказа, например 'Daily Resource Extraction'.

        Returns:
            Строка с типом заказа (например, 'daily_resource') либо пустая строка, если тип не распознан.
        """
        if self.is_event_commission():
            return 'daily_event'
    ...
    @Config.when(SERVER=None)
    def commission_name_parse(self, string):
        """Сопоставляет название заказа с его типом (сервер CN, стандартный резервный вариант).

        Сначала проверяет, относится ли заказ к событию, затем перебирает словарь CN-названий по ключевым словам.

        Args:
            string: Название заказа, например 'NYB VIP Escort'.
    ...
    def is_event_commission(self):
        """Определяет, относится ли заказ к событию.

        Проверяет цвет области в левой части карточки заказа. Различные события используют разные цветовые маркеры;
        в настоящее время в качестве критерия используется розово-жёлтый градиент события курорта (2023.04.27).

        Returns:
            bool: Относится ли заказ к событию.
        """
        # Текущий заказ события: розово-жёлтый градиент (стиль повтора Resort / события Idol Master)
    ...
    def convert_to_night(self):
        """Преобразует заказ категории extra в категорию night."""
    ...
    def convert_to_running(self):
        """Устанавливает статус заказа в running и сбрасывает время создания на текущее."""
        if self.valid:
            self.status = 'running'
            self.create_time = current_time()
    ...
    def finish_time(self):
        """Ожидаемое время завершения заказа.

        Returns:
            Время завершения выполняющегося заказа либо None для остальных состояний.
        """
