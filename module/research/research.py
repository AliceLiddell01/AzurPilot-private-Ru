...
class RewardResearch(ResearchSelector, ResearchQueue, StorageHandler):
    ...
    3. обработать отложенный проект серии T, требующий заказов;
    ...
    def handle_pending_t_research(self):
        """
        Обрабатывает отложенный проект Research серии T, требующий заказов.

        Для запуска Research серии T нужно выполнить заказы. Метод проверяет,
        есть ли такой ожидающий проект, и пытается его запустить. После успешного
        после запуска оставшееся количество записывается в конфигурацию, а модуль заказов
        выполняет их.
