"""Статистика бонусов кампании.

Распознает и анализирует всплывающее окно бонусных наград за прохождение кампании,
извлекая информацию о дополнительных предметах дропа и записывая ее в статистику.
Наследует GetItemsStatistics для повторного использования распознавания предметов.
"""
...
class CampaignBonusStatistics(GetItemsStatistics):
    ...
    def stats_get_items(self, image, **kwargs):
        """Извлекает предметы наград по скриншоту завершения кампании.

        Проверяет корректность скриншота по количеству монет: при слишком малом количестве или их отсутствии скриншот считается невалидным.

        Args:
            image (np.ndarray): Скриншот завершения кампании.

        Returns:
            list[Item]: Список предметов наград.
        """
    ...
    def revise_item(self, item):
        """Корректирует количество предметов при ошибках OCR-распознавания.

        Чипы в награде кампании выпадают по 9–30+ штук, но иногда 10 ошибочно распознается как 1.
        Также выполняется корректировка при аномальном количестве кораблей.

        Args:
            item (Item): Экземпляр предмета.

        Returns:
            Item: Скорректированный экземпляр предмета.
        """
