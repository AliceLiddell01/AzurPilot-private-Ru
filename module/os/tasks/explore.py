...
class OpsiExplore(OSMap):
    ...
    def _os_explore_task_delay(self):
        ...
        with self.config.multi_set():
            ...
            can_hazard1_leveling = (
                self.config.OpsiExplore_AllowHazard1Leveling
                and self.name_to_zone(self.config.OpsiExplore_LastZone).zone_id
                not in [0, 44, 24]
            )
    ...
    def _os_explore(self):
        def end():
            ...
            logger.info(
                '[大世界-探索] 如需重新运行，请清除 '
                'OpsiExplore.Scheduler.NextRun 并设置 '
                'OpsiExplore.OpsiExplore.LastZone=0'
            )
            with self.config.multi_set():
        ...
        full_order = [
            int(value.strip(' \t\r\n'))
            for value in self.config.OS_EXPLORE_FILTER.split('>')
        ]
        total_zones = len(full_order)
        try:
            last_zone = self.name_to_zone(
                self.config.OpsiExplore_LastZone
            ).zone_id
        except ScriptError:
            logger.warning(
                '[大世界-探索] 无效的 '
                f'OpsiExplore_LastZone={self.config.OpsiExplore_LastZone}, '
                '重新探索'
            )
            last_zone = 0
        ...
        if last_zone in full_order:
            ...
            if total_zones > 0:
                ...
                self.config.OpsiExplore_ExploreProgress = (
                    f'已完成百分之{percentage:.2f}'
                )
            logger.info(
                f'上次区域: {self.name_to_zone(last_zone)}, '
                f'next zone: {order[:1]}'
            )
        elif last_zone == 0:
            completed_count = 0
    ...
    def os_explore(self):
        ...
        failed_zone = [
            self.name_to_zone(zone)
            for zone in self._os_explore_failed_zone
        ]
        logger.error(
            '[大世界-每月开荒+] 以下区域开荒失败，请检查游戏设置和区域内未完成事件: '
            f'{failed_zone}'
        )
