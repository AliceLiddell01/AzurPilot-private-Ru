...
class OpsiDaily(OSMap):
    def os_port_mission(self):
        ...
        Pages:
            ...
            out: page_os, 大世界地图
        """
        logger.hr('Операция «Сирена» — ежедневные задания+: портовые задания', level=1)
        ...
        for port in ports:
            ...
            logger.hr(f'Операция «Сирена» — ежедневные задания+: порт={port}', level=2)
    ...
    def os_daily_clear_all_mission_zones(self):
        ...
        if get_os_reset_remain() > 0:
            logger.info('До сброса Операции «Сирена» более 1 дня, зачистка зон ежедневных заданий+ пропущена')
            return
        ...
        try:
            ...
            logger.warning('Недопустимая конфигурация зоны задания, зачистка зон ежедневных заданий+ пропущена')
            zones = []

        for zone in clear_zones:
            logger.hr(f'Операция «Сирена» — ежедневные задания+: зачистка зоны, zone_id={zone.zone_id}', level=1)
            try:
        ...
        while True:
            ...
            if self.zone.is_port and finished_combat == 0 and result in (
                ...
                if stuck_port_retry >= 3:
                    logger.warning(
                        f'Портовое задание Операции «Сирена»+, вероятно, застряло в зоне {zone_id} ({self.zone}); '
                        f'{stuck_port_retry} попыток автопоиска не дали результата, ежедневные задания+ остановлены во избежание бесконечного обновления зоны.')
    ...
    def os_daily(self):
        ...
        if self.config.OpsiDaily_SkipSirenResearchMission and self.config.SERVER not in ['cn']:
            logger.warning(f'[Операция «Сирена» — ежедневные задания] Пропуск задания исследования Сирен не поддерживается сервером {self.config.SERVER}.')
        ...
        if self.config.OpsiDaily_KeepMissionZone and self.config.SERVER not in ['cn']:
            logger.warning(f'[Операция «Сирена» — ежедневные задания] Сохранение зоны задания не поддерживается сервером {self.config.SERVER}.')
        ...
        if self.config.OpsiDaily_KeepMissionZone:
            if self.zone.is_azur_port:
                logger.info('[Операция «Сирена» — ежедневные задания] Уже в порту Азур Лейн')
            else:
