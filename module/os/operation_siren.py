...
class OperationSiren(
    ...
    def os_target_receive(self):
        ...
        if next_reset - now < timedelta(days=1):
            logger.error('[Операция «Сирена» — достижения] До следующего сброса остался всего один день, поэтому награды за достижения могут пропасть.'
                         'Сбор наград отложен до следующего сброса.')
        else:
            self.os_map_goto_globe(unpin=False)
            if self.appear(TARGET_RED_DOT):
                ...
                logger.info('[Операция «Сирена» — достижения] Нет доступных наград')
        self.config.OpsiTarget_LastRun = now.replace(microsecond=0)

    def _os_target(self):
        if self.config.OpsiTarget_LastRun > get_server_last_update('00:00'):
            logger.warning('Достижения зон сегодня уже проверялись; задача остановлена')
        else:
            logger.hr('Операция «Сирена» — достижения зон', level=1)
            self._os_target_enter()
    ...
    def os_daily(self):
        ...
        if self.config.OpsiDaily_CollectTargetReward:
            if self.server_support_os_target():
                ...
                logger.info(f'Сервер {self.config.SERVER} пока не поддерживает достижения зон; обратитесь к разработчикам')
