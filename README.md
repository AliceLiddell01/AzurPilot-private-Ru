...
Постоянная проверка `dev_tools/runtime_russianization_audit.py` анализирует только доказанные operator-facing consumer sites и fail-closed отклоняет новый CJK либо неклассифицированное английское предложение. Технические токены, пути, URL, package/game identifiers и deferred exception text не считаются переводом автоматически. Проверка одновременно защищает `ru-RU`, единственный сервер `en`, Global package, `assets/en`, EN metadata без foreign fallback и единственный публичный OCR namespace `azur_lane`.
