...
Старые locale-файлы пока остаются в репозитории как неактивные данные для контролируемого удаления на отдельном этапе. Полный перевод всех значений интерфейса выполняется отдельно; часть строк временно наследует bootstrap-содержимое.
