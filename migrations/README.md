...
пересчитать из известного вызывающему коду ключа, но runtime намеренно не
сохраняет параллельную legacy-ветку поиска после `0008`. Поэтому первый повтор
операции, впервые записанной до `0008`, создаст новую строку с исходным ключом;
историческая строка сохраняется без переписывания.
