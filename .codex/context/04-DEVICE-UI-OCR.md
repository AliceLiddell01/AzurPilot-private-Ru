...
- размер изображения;
- порядок каналов;
- канонический EN asset; asset другого региона проверяется только в явно заданной задаче совместимости;
- область crop;
- масштабирование/DPI;
- переходный кадр;
...
- варианты темы; варианты других регионов — только при явной задаче совместимости;
...
UI-driven сканирование флотов не должно считать один detector-positive кадр доказательством завершённого перехода. Переходные кадры и быстрые последовательные screenshot могут кратковременно выглядеть как целевое состояние, после чего следующий свежий кадр снова показывает предыдущую границу.
...
- после физического сбоя продолжать следующий флот можно только из восстановленной детерминированной границы Formation; при неизвестном UI состоянии выполнение останавливается fail-closed;
...
- разные значения OCR;
- варианты темы, если затронуты; fixtures других регионов нужны только при явной задаче на унаследованную совместимость;
