"""Контролируемый приёмочный запуск Windows/MuMu для recovery-цепочки.
...
реализацию. Искусственными остаются только две boundary-инъекции: первый health
result и graceful-stop result.
"""
...
class ForcedGracefulFailurePlatform:
    ...
    def emulator_stop(self):
        logger.warning(
            '[live recovery acceptance] Synthetic boundary: graceful stop считается '
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description='Контролируемая разрушительная приёмочная проверка recovery эмулятора.',
    )
...
def _require_runtime_safety(script: AzurLaneAutoScript):
    if sys.platform != 'win32':
        raise RuntimeError('Live recovery acceptance разрешён только на Windows.')

    config = script.config
    if not config.Error_GameStuckRestart or not config.Error_AdbOfflineRestart:
        raise RuntimeError(
            'Runtime profile не получил ожидаемые включённые по умолчанию recovery-настройки: '
    ...
    logger.info(
        f'[live recovery acceptance] Target подтверждён: instance={name}, id={instance_id}'
    )
    return platform
...
def _run_full_chain(script: AzurLaneAutoScript, platform) -> None:
    ...
    def injected_game_health():
        ...
        if health_calls == 1:
            logger.warning(
                '[live recovery acceptance] Synthetic boundary: первый реальный health '
    ...
    def injected_fault():
        raise GameStuckError('Контролируемая инъекция сбоя для live recovery acceptance')

    script.__dict__['emulator_recovery_live_fault'] = injected_fault
    ...
    try:
        with (
            ...
            outcome = script.run('emulator_recovery_live_fault', skip_first_screenshot=True)
    finally:
        script.__dict__.pop('emulator_recovery_live_fault', None)

    if outcome != 'recoverable':
        raise RuntimeError(f'Full-chain live recovery не завершилась успешно: outcome={outcome!r}')
    if health_calls != 2:
        raise RuntimeError(
            f'Ожидались две health boundary проверки, получено: {health_calls}'
        )
    ...
    logger.info('[live recovery acceptance] Full-chain hard-kill recovery PASS')
...
def _run_one_scheduler_task(script: AzurLaneAutoScript) -> ContinuationResult:
    ...
    logger.info(
        f'[live recovery acceptance] Scheduler continuation PASS: task={state.task}, outcome=True'
    )
    return state
...
def main(argv=None) -> int:
    ...
    try:
        ...
        logger.exception_context(
            title='Live recovery acceptance FAIL',
            exc=exc,
            impact='Recovery нельзя считать принятой на реальной MuMu-среде.',
    ...
    logger.info(
        '[live recovery acceptance] PASS: runtime default-on, production hard-kill recovery, '
