"""Автономный synthetic smoke recovery-цепочки эмулятора."""
...
def build_suite() -> unittest.TestSuite:
    transport = load_test_module(
        '_emulator_recovery_transport_tests',
        'tests/runtime/test_emulator_recovery_transport.py',
    )
    process = load_test_module(
        '_emulator_recovery_process_tests',
        'tests/platform/windows/test_mumu_process_control.py',
    )
    platform = load_test_module(
        '_emulator_recovery_platform_tests',
        'tests/platform/windows/test_platform_windows_recovery_behavior.py',
    )
    scheduler = load_test_module(
        '_emulator_recovery_scheduler_tests',
        'tests/application/test_alas_error_handling.py',
    )
    full_chain = load_test_module(
        '_emulator_recovery_full_chain_tests',
        'tests/runtime/test_emulator_recovery_full_chain.py',
    )
    continuation = load_test_module(
        '_emulator_recovery_scheduler_continuation_tests',
        'tests/runtime/test_emulator_recovery_scheduler_continuation.py',
    )
