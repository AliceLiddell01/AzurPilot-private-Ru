...
| G. Routing regression | обычный Game request загрузил только Development workflow или не использовал project-scoped `azurpilot-game` | вернуть Game operation в `azurpilot-game-control` и direct local stdio route; catalog/runtime/auth mismatch направить сюда; Development и Connected App не использовать как Game fallback. |
