...
| --- | --- | --- |
| A. Catalog drift | backend contract публикует capability, а текущая direct Codex session её не видит; сравнение count/fingerprint/exact surface расходится | stale client/plugin/session snapshot; не выполнять mutation и не переключаться на Connected App, выбрать один подтверждённый direct refresh, затем повторно проверить callable catalog и соответствующий contract. Вернуться к normal operation можно только при совместимости обоих источников. |
| B. Runtime drift | Git HEAD/source bundle новее, чем код работающего backend process | stale deployment/runtime; выполнить только owned `azur mcp restart` и подтвердить новый process/contract/catalog. |
...
| G. Routing regression | обычный Game request загрузил только Development workflow или не использовал project-scoped `azurpilot-game` | вернуть Game operation в `azurpilot-game-control` и явно выбранный first-class local route; catalog/runtime/auth mismatch направить сюда; Development и Connected App не использовать как Game fallback. |
...
client/plugin/session layer. Connected App не является direct Codex fallback. После доказанной синхронизации и совместимости
