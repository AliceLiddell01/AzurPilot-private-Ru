...
def test_network_cleanup_preserves_useful_features_and_removes_only_reviewed_defaults():
    ...
    # Пользовательский случайный фон WebUI намеренно сохраняется до появления собственного аналога.
    ...
    # Stage 3 полностью удаляет runtime Wiki provider событий.
