...
class _ViewStub:
    ...
    def predict_swipe(self, other, *, with_current_fleet, with_sea_grids):
        assert with_current_fleet is False
        assert with_sea_grids is True
        return (
...
def _camera(camera_points, update_results, recovery_steps):
    ...
    def ensure_edge_insight(**_kwargs):
        record, new_camera, new_view = next(steps)
        if new_camera is not None:
            instance.camera = new_camera
        if new_view is not None:
            instance.view.position = new_view
        return record
...
def test_full_scan_defers_view_when_camera_changes_without_observed_view_change():
    first = _Grid((0, 0))
    second = _Grid((5, 0))
    camera = _camera(
        [first, second],
        update_results=[False, True, True],
        recovery_steps=[([(1, 0)], (1, 0), None)],
    )
...
def test_full_scan_retries_same_view_immediately_after_confirmed_view_movement():
    first = _Grid((0, 0))
    second = _Grid((5, 0))
    camera = _camera(
        [first, second],
        update_results=[False, True, True],
        recovery_steps=[([(0, 0)], (1, 0), (1, 0))],
    )
...
def test_full_scan_exhausts_confirmed_view_movement_retry_limit():
    point = _Grid((0, 0))
