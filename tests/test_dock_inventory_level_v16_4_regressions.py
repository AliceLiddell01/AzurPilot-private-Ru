...
def _load_rgb(prefix: str, part_count: int, expected_sha256: str) -> np.ndarray:
    parts = [
        FIXTURE_DIR / f"{prefix}{index:02d}" for index in range(1, part_count + 1)
    ]
...
def test_v16_4_roon_trailing_artwork_is_excluded_from_numeric_region() -> None:
    prefix, part_count, digest, expected = CASES[0]
    assert expected == 125
    result = DockLevelOcr(
        (0, 0, 58, 31),
        name="TEST_V16_4_ROON_LEVEL",
        threshold=64,
    ).pre_process(_load_rgb(prefix, part_count, digest))
...
@pytest.mark.parametrize(
    ("prefix", "part_count", "fixture_sha256", "expected_level"),
    CASES[1:],
)
def test_v16_4_level1_artwork_cleanup_keeps_real_digit_evidence(
    ...
    result = DockLevelOcr(
        ...
        threshold=64,
    ).pre_process(_load_rgb(prefix, part_count, fixture_sha256))
...
def test_v16_4_level_failures_are_read_by_bundled_ocr() -> None:
    images = [
        _load_rgb(prefix, part_count, digest)
        for prefix, part_count, digest, _expected in CASES
    ]
    expected = [expected for _prefix, _count, _digest, expected in CASES]
...
def test_production_adapter_reads_real_v16_4_level_failures(
    ...
    result = DockLevelOcrAdapter().read_levels(
        _load_rgb(prefix, part_count, fixture_sha256),
        ((0, 0, 58, 31),),
    )
