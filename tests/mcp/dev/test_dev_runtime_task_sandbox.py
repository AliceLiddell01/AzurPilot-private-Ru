...
def test_session_keeps_legacy_task_lifecycle_defaults(tmp_path: Path) -> None:
    environment = _environment(tmp_path)
    manager = _manager(environment, _Backend())
...
def test_legacy_marker_without_task_fields_remains_compatible(tmp_path: Path) -> None:
    environment = _environment(tmp_path)
    legacy = DevSession(
