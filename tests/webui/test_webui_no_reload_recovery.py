...
class TestWebUiNoReloadRecovery(unittest.TestCase):
    def test_no_reload_starts_webui_without_runtime_recovery(self):
        with (
            patch.object(gui, "_configure_gui_logging"),
            patch.object(gui, "func") as func,
        ):
            self.assertTrue(gui._run_webui_without_reload())

        func.assert_called_once_with(None, None)

    def test_webui_process_stop_excludes_bot_runtime_children(self):
        process = Mock()
        with patch.object(gui, "_stop_process_tree", return_value=True) as stop_tree:
            self.assertTrue(gui._stop_webui_process_tree(process))

        stop_tree.assert_called_once_with(process, "WebUI", include_children=False)
