...
- tracked-исходники и диапазон коммитов PR через Gitleaks `8.30.1` с проверкой SHA256 загружаемого архива;
...
Source-scan Gitleaks выполняется по временному snapshot, собранному из `git ls-files`, поэтому локальные ignored-файлы вроде `.env`, `log/`, `config/state/`, `.venv` и `.tools` не триггерят проверку. Это не allowlist для репозитория: исторический `gitleaks git` продолжает проверять содержимое Git-истории, а отдельный gate запрещает добавление `.env`, credentials, логов и других рискованных файлов. Для локальной проверки staged-изменений используйте `gitleaks git --pre-commit`; запуск `gitleaks dir .` намеренно не является проектным CI-эквивалентом.
