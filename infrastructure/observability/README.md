...
никогда не открывается без caller auth. Значение читают два независимых
потребителя: Compose берёт его из `.env`, а MCP-клиент — из окружения своего
процесса, потому что клиентская регистрация указывает только имя переменной и
`.env` не читает. Дополнительно healthcheck Grafana требует ответ `401` на
анонимный `POST /mcp`, поэтому выключенный caller auth становится наблюдаемым
как `unhealthy`, а не остаётся невидимым. Клиентская регистрация передаёт только
